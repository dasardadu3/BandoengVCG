...
def song(client, message):
    ...
    for i in message.command[1:]:
        query += ' ' + str(i)
    print(query)
    m = message.reply('🔍 Kela dagoan keur neang lagu...')
    ...
    try:
        ...
        m.edit(
            "❌ Laguna teu kapanggih anying.\n\n Coba ngetikna sing bener atau make link lagu na, Ketik /help bisi maneh teu ngarti."
        )
        print(str(e))
        return
    m.edit("Sedang mendongdot lagu")
    try:
        with youtube_dl.YoutubeDL(ydl_opts) as ydl:
            info_dict = ydl.extract_info(link, download=False)
            audio_file = ydl.prepare_filename(info_dict)
            ydl.process_info(info_dict)
        rep = '**🎵 Reupload by ** @developerantibucin'
...
async def ytmusic(client,message: Message):
    ...
    if not urlissed:
        await pablo.edit("Cmd perintah tidak valid, silahkan tekan /help kalo masih ga ngerti !")
        return
    ...
    await client.send_video(message.chat.id, video = open(file_stark, "rb"), duration = int(ytdl_data["duration"]), file_name = str(ytdl_data["title"]), thumb = sedlyf, caption = capy, supports_streaming = True , progress=progress, progress_args=(pablo, c_time, f'`Uploading {urlissed} lagu dari YouTube !`', file_stark))
    await pablo.delete()
    is_downloading = False
